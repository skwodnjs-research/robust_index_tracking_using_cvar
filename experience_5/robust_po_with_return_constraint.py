...
def solve_robust_po_with_return_const(
    stock_returns,
    alpha=0.95,
    delta = 0.01,
):
    ...
    mu_hat = np.mean(xi, axis=0)
    w_equal = np.ones(m) / m
    equal_robust_return = (
        mu_hat @ w_equal
        - delta * np.linalg.norm(w_equal, 2)
    )
    ...
    constraints = [
        ...
        mu_hat @ w - delta * cp.norm(w, 2) >= equal_robust_return,
    ]
...
for rebalance_month in rebalance_months:
    ...
    try:
        w_value, objective_value, status = solve_robust_po_with_return_const(
...
weight_history.to_csv(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_weights.csv")
optimization_history.to_csv(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_optimization_history.csv")
backtest_returns.to_csv(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_returns.csv")

print("\nFiles saved")

print(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_weights.csv")
print(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_optimization_history.csv")
print(f"../data/exp5_data_{index}_robust_po_lasso_with_return_const_alpha={alpha}_delta={delta}_returns.csv")