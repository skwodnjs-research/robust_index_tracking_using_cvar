...
for rebalanceYear = rebalanceYears
    ...
    [wOptimal, objectiveValue, exitflag] = optimizePortfolio(xi, alpha, tau, h);
...
weightFile = fullfile( ...
    "data", ...
    "data_" + index + ...
    "_robust_ceir_kde_gaussian_kl_weights.csv" ...
);

optimizationFile = fullfile( ...
    "data", ...
    "data_" + index + ...
    "_robust_ceir_kde_gaussian_kl_optimization_history.csv" ...
);

returnsFile = fullfile( ...
    "data", ...
    "data_" + index + ...
    "_robust_ceir_kde_gaussian_kl_returns.csv" ...
);
...
function [wOptimal, objectiveValue, exitflag] = optimizePortfolio( ...
    ...
    % 변수 범위
    lb = [zeros(m,1); -Inf; -Inf; 1e-8];
    ub = [ones(m,1);  Inf;  Inf; Inf];
    
    % 목적함수
    objective = @(x) robustObjective(x,xi,alpha,tau,h);
