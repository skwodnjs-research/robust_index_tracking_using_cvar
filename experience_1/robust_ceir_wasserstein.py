...
def solve_robust_ceir_wasserstein(
    ...
    L = xi_min - margin
    U = xi_max + margin     # 종목별로 범위를 다르게 지정하는 것도 가능할 것 같다.
    ...
    for i in range(n):
        ...
        # Dual-norm constraints (l2 norm is self-dual)
        constraints += [
            cp.norm(z_i1, 2) <= lamb,
            cp.norm(z_i2, 2) <= lamb,
        ]
...
weight_history.to_csv(f"data/data_{index}_robust_ceir_wasserstein_weights.csv")
optimization_history.to_csv(f"data/data_{index}_robust_ceir_wasserstein_optimization_history.csv")
backtest_returns.to_csv(f"data/data_{index}_robust_ceir_wasserstein_returns.csv")

print("\nFiles saved")

print(f"data/data_{index}_robust_ceir_wasserstein_weights.csv")
print(f"data/data_{index}_robust_ceir_wasserstein_optimization_history.csv")
print(f"data/data_{index}_robust_ceir_wasserstein_returns.csv")