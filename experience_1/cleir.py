...
def solve_cleir(
    ...
    # 목적함수
    objective = cp.Minimize(gamma + cp.sum(z) / (n * (1 - alpha)))
...
for rebalance_year in rebalance_years:
    ...
    rng = np.random.default_rng(SEED + rebalance_year)
    selected_columns = rng.choice(
        valid_columns,
        size=min(SELECTED, len(valid_columns)),
        replace=False,
    )
    ...
    try:
        w_value, objective_value, status = solve_cleir(
...
weight_history.to_csv(f"data/data_{index}_cleir_weights.csv")
optimization_history.to_csv(f"data/data_{index}_cleir_optimization_history.csv")
backtest_returns.to_csv(f"data/data_{index}_cleir_returns.csv")

print("\nFiles saved")

print(f"data/data_{index}_cleir_weights.csv")
print(f"data/data_{index}_cleir_optimization_history.csv")
print(f"data/data_{index}_cleir_returns.csv")