...
def solve_robust_ceir_wasserstein(
    stock_returns,
    index_returns,
    alpha=0.95,
    delta = 0.01,
    ...
    xi_max = np.nanmax(xi, axis=0)
    xi_min = np.nanmin(xi, axis=0)
    margin = 0.05   # 0.01 * (간격) 으로 했을 때 이상해짐, 0.01 값으로 했을때도 이상해짐, 0.05로 했을 때 안이상해짐
    L = xi_min - margin
    U = xi_max + margin

    print(np.min(L), np.max(U))
    ...
    z1 = cp.Variable((n, m+1))
    z2 = cp.Variable((n, m+1))
    nu1 = cp.Variable((n, m+1))
    nu2 = cp.Variable((n, m+1))
    ...
    for i in range(n):
        ...
        support_i1 = cp.sum(cp.maximum(cp.multiply(L, nu_i1),cp.multiply(U, nu_i1)))

        constraints += [
            gamma + support_i1 - z_i1 @ xi[i, :] <= s[i],
            z_i1 - nu_i1 == 0,
        ]
        ...
        support_i2 = cp.sum(cp.maximum(cp.multiply(L, nu_i2),cp.multiply(U, nu_i2)))

        constraints += [
            gamma - gamma / (1 - alpha) + support_i2 - z_i2 @ xi[i, :] <= s[i],
            z_i2 - nu_i2 == w_tilde / (1 - alpha),
        ]
...
for rebalance_year in rebalance_years:
    ...
    try:
        w_value, objective_value, status = solve_robust_ceir_wasserstein(
...
weight_history.to_csv(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_weights_case2.csv")
optimization_history.to_csv(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_optimization_history_case2.csv")
backtest_returns.to_csv(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_returns_case2.csv")

print("\nFiles saved")

print(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_weights_case2.csv")
print(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_optimization_history_case2.csv")
print(f"../data/data_{index}_wass_ceir_alpha={alpha}_delta={delta}_returns_case2.csv")