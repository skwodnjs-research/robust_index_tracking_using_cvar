...
예상했던 대로 굉장히 빠른 속도로 동일한 계산을 수행하는 모습을 확인하였다.

# Experience 4

실험 3을 기반으로, 1년치 데이터를 이용하여 계산된 포트폴리오를 한 달 동안 사용하는 방식으로 코드를 구현하였다. v1은 주식 100개를 선택하여 해당 주식으로 포트폴리오를 계산한 버전, v2는 모든 주식을 사용하여 포트폴리오를 계산한 버전이다.

### 실험 결과

실험 3에 의해, 계산 속도가 빨라진 덕분에 100개로 제한하는 방식이 아니더라도 계산하는데 부담이 없었다. 주식 수를 제한한 버전의 수익률이 더 높았는데, 이는 수익률이 높은 좋은 주식들을 선택한 결과일 수 있기 때문에 알고리즘의 성능으로 평가할 수는 없다.

주식 수를 제한하지 않고 계산하는 경우, 거의 모든 주식을 전부 사용해버리는 현상을 볼 수 있었다. 거래 수수료 등을 고려했을 때 sparse한 포트폴리오를 구하는 알고리즘이 필요한 상황이다.
