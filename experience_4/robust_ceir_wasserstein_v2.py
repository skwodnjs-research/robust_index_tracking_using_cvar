...
def solve_robust_ceir_wasserstein(
    ...
    # 최적화 변수
    w = cp.Variable(m)
    # z = cp.Variable(m, boolean=True)  # 종목 선택 여부
...
for rebalance_month in rebalance_months:
    ...
    # 학습 데이터 설정
    training_returns = training_returns.loc[:, valid_columns].fillna(0.0)
    ...
    weights = pd.Series(w_value, index=valid_columns, name=rebalance_date)

    # 향후 1개월 포트폴리오 수익률 계산
    holding_returns = stock_returns.loc[holding_dates, valid_columns]
...
weight_history.to_csv(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_weights_v2.csv")
optimization_history.to_csv(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_optimization_history_v2.csv")
backtest_returns.to_csv(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_returns_v2.csv")

print("\nFiles saved")

print(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_weights_v2.csv")
print(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_optimization_history_v2.csv")
print(f"../data/exp4_data_{index}_wass_ceir_alpha={alpha}_delta={delta}_returns_v2.csv")